...
class AsarPackager {
  ...
  async createPackageFromFiles(fileSet, packager) {
    ...
    // const rootForAppFilesWithoutAsar = path.join(this.destination, "app"); //拼接文件地址
    const rootForAppFilesWithoutAsar = this.destination; //拼接文件地址
    ...
    for (let i = 0, n = fileSet.files.length; i < n; i++) {
      ...
      if (isUnpacked) {
        ...
        copyFileOrData(fileCopier, newData, file, unpackedFile, stat)
